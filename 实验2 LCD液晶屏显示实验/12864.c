...
void pcb_GPIO_Init(void)
 { 
  ...
  RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOG|RCC_AHB1Periph_GPIOF, ENABLE);//那1?邦GPIOF那㊣?車

  //GPIOF9,F103?那??‘谷豕??
   GPIO_InitStructure.GPIO_Pin = GPIO_Pin_14 | GPIO_Pin_15;
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_OUT;//??赤“那?3??㏒那?
...
void write_figer(unsigned char y_add , unsigned char x_add , unsigned int figer)//?迆豕?o?????D∩那y℅?
{
    ...
    if(i==4)
			send_dat(0x30);//那y?Y℅～赤那㏒?℅?㊣?﹞⊿?赤

    for(j=i;j<5;j++)
    {
       send_dat(d[j]+0x30);//豕?米?米?那y℅??車谷?0x30辰2?∩米?米???那y℅?米?ASCII??㏒??迄????那y℅?﹞⊿?赤豕ㄓ??那?
    }
...
{  
    ...
    Delay(0xfff);
    send_cmd(0x30);
    send_cmd(0x0C);//0000,1100 ??足???那?㏒?車?㊣那off㏒?車?㊣那????off
    send_cmd(0x01);//0000,0001 ??DDRAM
    send_cmd(0x02);//0000,0010 DDRAM米??﹞1谷??
    send_cmd(0x80);//1000,0000 谷豕?“DDRAM 7??米??﹞000㏒?0000米?米??﹞??那y?¯A
}


/******************* (C) COPYRIGHT 2011 ﹞邦?﹞STM32 *****END OF FILE****/